...
Write-Host "$PSScriptRoot\azsql.powershell\private\classes\"

$classes = Get-ChildItem -Path $PSScriptRoot\..\azsql.powershell\private\classes\*.ps1

foreach( $class in $classes )
{
    $content = Get-Content -Path $class.FullName -Raw

    $null = $sb.AppendLine( "# class file - src\azsql.powershell\private\classes\$($class.Name)" )
...
$sb.ToString() | Set-Content -Path "$PSScriptRoot\..\azsql.powershell\azsql.powershell.psm1"
